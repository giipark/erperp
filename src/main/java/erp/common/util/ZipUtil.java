...
public class ZipUtil {
    ...
    public static void zip(String sourcePath, String output) throws Exception {
        ...
        if (!sourceFile.isFile() && !sourceFile.isDirectory()) {
            throw new Exception(messageSource.getMessage("lang804", null,LocaleContextHolder.getLocale()));
        }

        // output 의 확장자가 zip이 아니면 리턴한다.
        if (!(StringUtils.substringAfterLast(output, ".")).equalsIgnoreCase("zip")) {
            throw new Exception(messageSource.getMessage("lang803", null,LocaleContextHolder.getLocale()));
        }
     ...
     * @throws IOException
     */
    public static int unzip(FileInputStream inputZipFis, File outputFolder) throws IOException {
        int count=0;
        FileInputStream fis = null;
        ZipArchiveInputStream zis = null;
        FileOutputStream fos = null;
